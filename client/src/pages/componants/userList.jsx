...
export default function UserList({ setShowForm }) {
  ...
  // ❌ REMOVE THIS LINE - Don't store users in localStorage like this
  // localStorage.setItem('users', users); // This will cause issues
  ...
  const confirmDelete = async (id) => {
    ...
    try {
      ...
      const successMessage = response?.data?.deleteUser?.name 
        ? `${response.data.deleteUser.name} deleted successfully!`
      ...
      <div className="bg-white border border-slate-100 shadow-sm overflow-hidden">
                  ...
                  <tr key={i} className="border-b border-slate-50 hover:bg-slate-50 transition-colors">
                    ...
                    <td className="px-6 py-4">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <div className="font-semibold text-[#0d2145] text-sm">
                            {user.name}
                          </div>
                        </div>
                      </div>
                    ...
                    <td className="px-6 py-4">
                      <span
                        className={`text-xs font-medium px-3 py-1 rounded-full ${
                          DEPT_COLORS[user.department] || "bg-slate-100 text-slate-600"
                        }`}
                      >
                        {user.department}
                      </span>
                    ...
                    <td className="px-6 py-4">
                      <span
                        className={`text-xs font-medium px-3 py-1 rounded-full ${
                          ROLE_COLORS[user.jobRole] || "bg-slate-100 text-slate-600"
                        }`}
                      >
                        {user.jobRole}
                      </span>
                    ...
                    <td className="px-6 py-4">
                      <div className="flex items-center gap-2">
                        ...
                        >
                          {deleteId === user._id ? (
                            <img className="w-4" src="/barLoading.gif" alt="loading" />
                          ) : (
                            "Delete"
                          )}
                        </button>
        ...
        <div className="md:hidden divide-y divide-slate-100">
          {loading && (
            <div className="flex justify-center">
              <img className="" src="/loading.gif" alt="loading" width={150} />
            </div>
          )}
          {!loading &&
            currentItems.map((user, i) => (
              <div key={i} className="px-5 py-4">
                <div className="flex items-center justify-between mb-3">
                  ...
                  <div className="flex gap-2">
                    <button className="text-xs font-semibold text-[#0d2145] bg-[#0d2145]/10 px-3 py-1.5 rounded-lg">
                      Edit
                    </button>
                    <button
                      onClick={() => handleDelete(user)}
                      className="text-xs flex justify-center w-15 font-semibold text-[#e8192c] bg-[#e8192c]/10 px-3 py-1.5 rounded-lg"
                    >
                      {deleteId === user._id ? (
                        <img className="w-4" src="/barLoading.gif" alt="loading" />
                      ) : (
        ...
        <div className="px-6 py-3 bg-slate-50 border-t border-slate-100 flex items-center justify-between">
          <span className="text-xs text-slate-400">
            Showing {users.length} of {users.length} employees
          </span>
          {/* Pagination buttons */}
          {totalPages > 1 && (
            <div className="text-xs flex items-center gap-2 justify-center">
              <button
                className="text-xs px-3 py-1.5 rounded-lg bg-white border border-slate-200 text-slate-500 disabled:opacity-40 disabled:cursor-not-allowed"
                ...
                <button
                  className={`text-xs px-3 py-1.5 rounded-lg font-semibold ${
                    currentPage === page
                      ...
                      : "bg-slate-100 text-[#0d2145] hover:bg-slate-200"
                  }`}
                  key={page}
